...
reqeired_min=[2000,50,20]
reqeired_max=[2500,200,70]
...
def objective_function(amounts):
    ...
    penalty=0 
    for i in range(len(nutrients)):
        if nutrient_totals[i]<reqeired_min[i]:
            penalty+=(reqeired_min[i]-nutrient_totals[i])**2*100
        if nutrient_totals[i]>reqeired_max[i]:
            penalty+=(reqeired_min[i]-nutrient_totals[i])**2*100
