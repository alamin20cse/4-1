...
class RouteFindingAgent:
        ...
        while queue:
            ...
            for neighbor in self.env.get_neighbors(current):
                if neighbor not in visited:
                    queue.append((neighbor, path + [neighbor]))  # Fixed missing tuple parentheses
    ...
    def calculate_cost(self, path):
        ...
        for i in range(len(path) - 1):
            from_city = path[i]
            to_city = path[i + 1]
            cost = self.env.get_cost(from_city, to_city)  # Corrected method name
            total_cost += cost
        return total_cost  # Fixed indentation (was inside the loop before)
